...
В **Gateway режиме** **первый порт** в `PORT_FORWARD` (EXTERNAL_PORT) **ОБЯЗАТЕЛЬНО должен быть опубликован** в секции `ports:` файла `docker-compose.yml`. Иначе интернет-клиенты не смогут подключиться!

**Пример**:
```yaml
# В docker-compose.yml:
ports:
  - "8989:8989"  # ✅ ОБЯЗАТЕЛЬНО - совпадает с первым портом PORT_FORWARD
  - "443:443"    # ✅ ОБЯЗАТЕЛЬНО - совпадает с первым портом PORT_FORWARD
