...
### Мониторинг и надёжность (НОВОЕ в v2.1.4!)
- **📝 Комплексное логирование**: Логи мониторинга сервисов и отслеживания подключений
- **🔄 Автоматическое восстановление**: Автоматическое восстановление отсутствующих правил iptables/socat
- **💚 Проверки здоровья**: Непрерывный мониторинг доступности сервисов (каждые 30 секунд)
- **📊 Отображение имён контейнеров**: Показывает оригинальные имена контейнеров в логах
- **🔁 Ротация логов**: Автоматическая ротация логов (лимит 10MB, 5 исторических файлов)
- **⚠️ Отслеживание состояния**: Обнаруживает и логирует изменения состояния сервисов
...
## 📝 Система логирования (НОВОЕ в v2.1.4!)

ZeroTier Sidecar включает комплексное логирование для мониторинга здоровья сервисов и отслеживания подключений:

### Файлы логов

Все логи хранятся в `/var/log/zerotier-sidecar/` внутри контейнера:

| Файл лога | Назначение | Содержимое |
|-----------|-----------|------------|
| `monitor.log` | Мониторинг сервисов | Статус сервисов, проверки здоровья, восстановление правил |
| `connections.log` | Отслеживание подключений | Новые TCP подключения с временными метками |

### Примеры логов

**Лог мониторинга** (`/var/log/zerotier-sidecar/monitor.log`):
```
...
**Лог подключений** (`/var/log/zerotier-sidecar/connections.log`):
```
...
- **📋 Имена контейнеров**: Логи показывают оригинальные имена контейнеров (например, `rsync-server`) вместо просто IP адресов
- **⏰ Временные метки**: Все записи включают точные временные метки в формате `YYYY-MM-DD HH:MM:SS`
- **🔁 Автоматическая ротация**: Файлы логов автоматически ротируются при достижении 10MB
- **📚 Сохранение истории**: Хранится до 5 ротированных файлов логов (например, `monitor.log.1`, `monitor.log.2`, ...)
- **⚡ Ограничение частоты**: Подключения логируются с частотой до 3/минуту на порт для предотвращения переполнения логов
- **🎯 Отслеживание состояния**: Логирует только изменения состояния (DOWN → UP) для уменьшения шума

### Доступ к логам

**Монтирование тома для персистентных логов**:
```yaml
volumes:
  - ./sidecar-logs:/var/log/zerotier-sidecar  # Логи выживают при перезапуске контейнера
```

**Просмотр логов в реальном времени**:
```bash
# Мониторинг логов сервисов
docker exec zerotier-sidecar tail -f /var/log/zerotier-sidecar/monitor.log

# Мониторинг логов подключений
...
**Копирование логов на хост для анализа**:
```bash
# Копировать текущие логи
docker cp zerotier-sidecar:/var/log/zerotier-sidecar ./logs

# Или использовать volume mount (рекомендуется)
# Логи будут доступны в ./sidecar-logs/ на хосте
```

### Что мониторится

**Мониторинг сервисов** (каждые 30 секунд):
- ✅ Доступность сервиса (TCP проверка подключения)
- ✅ Наличие правил iptables
- ✅ Процессы socat (для Gateway режима)
- ✅ Автоматическое восстановление отсутствующих правил
- ✅ Обнаружение изменений состояния сервисов

**Логирование подключений** (в реальном времени):
- ✅ Новые TCP подключения
- ✅ Исходный IP адрес
- ✅ Целевой IP и порт
- ✅ Какой сервис получает доступ

### Ротация логов

Логи автоматически ротируются для предотвращения заполнения диска:

```
/var/log/zerotier-sidecar/
├── monitor.log         # Текущий файл (до 10MB)
├── monitor.log.1       # Предыдущая ротация
├── monitor.log.2
├── ...
├── monitor.log.5       # Самый старый (затем удаляется)
├── connections.log     # Текущий файл (до 10MB)
├── connections.log.1
└── ...
```
...
# Просмотр логов мониторинга (НОВОЕ в v2.1.4!)
docker exec zerotier-sidecar tail -f /var/log/zerotier-sidecar/monitor.log

# Просмотр логов подключений (НОВОЕ в v2.1.4!)
docker exec zerotier-sidecar tail -f /var/log/zerotier-sidecar/connections.log
```
