...
- **🏷️ Резолвинг имен контейнеров**: Используйте имена контейнеров вместо IP в правилах проброса портов
- **🔍 Умное управление DNS**: Сохраняет Docker embedded DNS для бесшовного обнаружения сервисов
...
# Проброс портов: ВНЕШНИЙ_ПОРТ:IP_ИЛИ_ИМЯ_НАЗНАЧЕНИЯ:ПОРТ_НАЗНАЧЕНИЯ
# Множественные порты разделяются запятой
# Можно использовать имена контейнеров или IP адреса!
PORT_FORWARD=873:my-rsync-server:873,22:my-ssh-server:22
# Или с IP: PORT_FORWARD=873:172.26.0.3:873,22:172.26.0.4:22
```
...
Переменная `PORT_FORWARD` использует формат: `ВНЕШНИЙ_ПОРТ:IP_ИЛИ_ИМЯ_НАЗНАЧЕНИЯ:ПОРТ_НАЗНАЧЕНИЯ`

- **ВНЕШНИЙ_ПОРТ**: Порт, доступный из ZeroTier сети
- **IP_ИЛИ_ИМЯ_НАЗНАЧЕНИЯ**: IP целевого Docker контейнера или имя контейнера
- **ПОРТ_НАЗНАЧЕНИЯ**: Порт целевого контейнера

**Примеры**:
- Использование имен контейнеров (рекомендуется): `873:my-rsync-server:873,22:my-ssh-server:22`
- Использование IP: `873:172.26.0.3:873,22:172.26.0.4:22`
- Смешанный вариант: `873:my-service:873,22:172.26.0.4:22,80:nginx:8080`

**Резолвинг имен контейнеров**:
- Имена контейнеров автоматически преобразуются в IP при запуске
- Работает с контейнерами в одной Docker сети
- Использует Docker embedded DNS (127.0.0.11) для надежного резолвинга
- Автоматический fallback на системный DNS при необходимости
